...
const DeviceBatchUploadDialog: FC<IProps> = (props) => {
  const { onClose, open } = props

  const [downloadTemplate] = Api.useDownloadTemplateDeviceUploadMutation()
  const [batchUpload] = Api.useBatchUploadDevicesMutation()
  ...
  const handleDropFiles = (files: File[], fileRejections?: FileRejection[], event?: DropEvent) => {
    ...
    if (fileRejections && fileRejections?.length > 0) {
      toast.error('Files must be the supported and valid file format.')
    } else {
      if (files.length > 0) {
        batchUpload(files[0])
          .unwrap()
          .then((res) => {
            toast.success('Devices uploaded')
            onClose()
          })
          .catch((err) => {
    ...
    downloadTemplate()
      ...
      .then((res) => {
        if (res) {
          ...
          FileSaver.saveAs(blob, 'Device_batch upload template.xlsx')
        }
      })
      .catch((err) => {
  ...
  return (
    <DialogWrapper label={'Batch Upload Device'} onClose={onClose} open={open} maxWidth={'802px'}>
...
export default DeviceBatchUploadDialog
