...
import ReportOverviewChart from './ReportOverviewChart'
...
const LEGENDS = [
  { label: 'Audit Data', color: '#F64E60' },
  { label: 'Device IoT Notification', color: '#0BB783' },
  { label: 'Maintenance Data', color: '#3699FF' },
  { label: 'Incident Data', color: '#8950FC' },
  { label: 'Feedback Data', color: '#A5742B' },
]
...
const ReportCharts: FC<IProps> = ({ projectIds, locationIds, reportTypes }) => {
  ...
  const { chartData, counts } = useMemo(() => {
    ...
    if (reports && reports?.length > 0) {
      ...
      for (const item of sortedReports || []) {
        ...
        const audit = item.count.audit
        const incident = item.count.incident
        const deviceIoTNotification = item.count.deviceIoTNotification
        const maintenance = item.count.maintenance
        const feedback = item.count.feedback
        const received = item.count.received

        totalReceivedCount += received
        totalFeedbackCount += audit
        totalAuditCount += maintenance
        totalIncidentCount += incident
    ...
    const counts = [
      { label: 'Total IoT alert received', value: totalReceivedCount },
      { label: 'Total Incident', value: totalIncidentCount },
      { label: 'Total feedback', value: totalFeedbackCount },
      { label: 'Total audit done', value: totalAuditCount },
    ]
  ...
  const legendList = useMemo(() => {
    return LEGENDS.filter((_e, idx) => reportTypes.length === 0 || reportTypes.includes(idx))
  }, [reportTypes])
      ...
      <Box sx={{ px: 5, py: 3 }}>
        <Grid container columnSpacing={0} rowSpacing={2}>
          <Grid item lg={9} xs={12}>
            <ReportOverviewChart data={chartData} reportTypes={reportTypes} />
          </Grid>
                    ...
                    <Box key={idx} sx={{ display: 'flex', alignItems: 'center', gap: 1.25 }}>
                      <Grid container spacing={1} alignItems={'center'}>
                        ...
                        <Grid item xs={6}>
                          ...
                          >
                            <Typography typography='h3' sx={{ fontWeight: 500, color: 'grey.800' }}>
                              {count.value}
                            </Typography>
                          </Box>
                        </Grid>
