...
import { QueryTypes } from '@reduxjs/toolkit/dist/query/endpointDefinitions'
...
interface IProps {
  ...
  queryApiKey?:
    ...
    | 'useGetAreaListQuery'
    | 'useGetUnitListQuery'
    | 'useGetAuditFormTypeListQuery'
    | 'useGetProjectListQuery'
    | 'useGetAuditFormulaGroupListQuery'
    | 'useGetRoleListQuery'
    | 'useGetUserListQuery'
    | 'useGetDeviceTypeListQuery'
    | 'useGetAuditRatingTemplateListQuery'
    | 'useGetDeviceListQuery'
    | 'useGetIncidentTypeListQuery'
    | 'useGetTaskPremiseListQuery'
    | 'useGetTaskStaffListQuery'
    | 'useGetTaskActivitiesListQuery'
    | 'useGetSopsQuery'
    | 'useGetDeviceTypeQuery'
    | 'useGetTaskActivitiesQuery'
    | 'useGetRobotGetAvailableTasksQuery'
  queryFilters?: any
...
const InfiniteMultipleSelect: FC<IProps> = ({
  selectedItems,
  limit = 10,
  onChange,
  labelForAll,
  ...
  const handleSelect = (item: ISelectItem, categoryItem?: ISelectItem) => {
    ...
    if (isSingleSelect) {
      ...
      const idx = newItems.findIndex((_item) => _item.value === item.value)
      if (idx >= 0) {
        newItems.splice(idx, 1)
      } else {
        newItems.push(item)
        newItems = categoryItem
          ? newItems.filter((t) => !t.isCategory || categoryItem.value !== t.value)
          : newItems
      }
      onChange(newItems)
    }
  ...
  const handleSelectCat = (item: ISelectItem, items: ISelectItem[]) => {
    ...
    if (isSingleSelect) {
      ...
      const subItemValues = items.map((t) => t.value)
      const newItems = [...selectedItems].filter(
        (_item) => _item.isCategory || !subItemValues.includes(_item.value)
      )
      const idx = newItems.findIndex((_item) => _item.isCategory && _item.value === item.value)
        ...
        <Paper sx={{ py: 1.25, px: 1.5, minWidth: ref.current?.clientWidth || ' 100%' }}>
          ...
          >
            {items.length > 0 && (
              <>
                {!withCategory &&
                  (allowAllSelect ? [allSelect, ...items] : items).map((item) => {
                    const isSelected =
...
export default InfiniteMultipleSelect
