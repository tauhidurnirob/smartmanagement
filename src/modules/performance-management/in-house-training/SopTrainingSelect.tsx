...
import { ISopTraining } from '../../../types/performance-management'
...
const SopTrainingSelect = ({
  ...
  const handleParseItem = (item: ISopTraining) => {
    return {
      value: item.id,
      label: item.sopName,
    }
  ...
  return (
    <Box sx={{ ...(sx || {}) }}>
      {!hiddenLabel && <FilterLabel text='SOP Training' className='label' />}
      <InfiniteMultipleSelect
        selectedItems={selected}
        onChange={handleChange}
        limit={10}
        labelForAll={
          placeholder ? placeholder : disableAllSelect ? 'Select SOP Training' : 'All SOP Trainings'
        }
...
export default SopTrainingSelect
