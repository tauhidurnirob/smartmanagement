...
import MaintenanceReportChart from './MaintenanceReportChart'
import { DOWNLOAD_FILE_TYPES } from '../../../helpers/constants'

const reports = [
  { date: '2023-06-14T10:10:00.000000Z', count: { failed: 2, robot: 35, sensor: 5, nonIot: 8 } },
  { date: '2023-06-15T10:10:00.000000Z', count: { failed: 2, robot: 35, sensor: 5, nonIot: 8 } },
  { date: '2023-06-16T10:10:00.000000Z', count: { failed: 2, robot: 35, sensor: 5, nonIot: 8 } },
  { date: '2023-06-17T10:10:00.000000Z', count: { failed: 2, robot: 35, sensor: 5, nonIot: 8 } },
  { date: '2023-06-18T10:10:00.000000Z', count: { failed: 2, robot: 35, sensor: 5, nonIot: 8 } },
]

const LEGENDS = [
  { label: 'Failed Maintenance', color: '#F64E60' },
  { label: 'Robot Maintenance', color: '#0BB783' },
  { label: 'Sensor Maintenance', color: '#3699FF' },
  { label: 'Non IoT Maintenance', color: '#8950FC' },
]
...
const MaintenanceReportsCharts: FC<IProps> = ({ projectIds, locationIds }) => {
  ...
  const { chartData, counts } = useMemo(() => {
    ...
    if (reports && reports?.length > 0) {
      ...
      for (const item of sortedReports || []) {
        ...
        const failedCount = item.count.failed
        const robotCount = item.count.robot
        const sensorCount = item.count.sensor
        const nonIotCount = item.count.nonIot
        const totalCount = failedCount + robotCount + sensorCount + nonIotCount
        ...
        sensor += sensorCount
        nonIot += nonIotCount
        totalCompleted = robotCount + sensorCount + nonIotCount
        totalOverdue = failedCount
      ...
      avgWeekMaintenance = total / totalWeeks
    }

    const counts = [
      { label: 'Total Scheduled Maintenance', value: total },
      { label: 'Total Completed Maintenance', value: totalCompleted },
      { label: 'Total Overdue Maintenance', value: totalOverdue },
      { label: 'Average Maintenance per Week', value: avgWeekMaintenance.toFixed(1) },
    ]
      ...
      <Box sx={{ px: 5, py: 3 }}>
        <Grid container columnSpacing={0} rowSpacing={2}>
          <Grid item lg={9} xs={12}>
            <MaintenanceReportChart data={chartData} selectedTypes={[]} />
          </Grid>
...
export default MaintenanceReportsCharts
