...
import DeviceFloorMap from '../../modules/device/DeviceFloorMap'
import { DEVICE_STATUS_LIST, EDeviceMapType } from '../../helpers/constants'
import { Plus } from '../../assets/icons/plus'
import { tmpDeviceList } from '../../modules/device/dummy'
...
import DeviceActivityListModal from './DeviceActivityListModal'
...
const DeviceControlDetailOverview = () => {
  const [devices, setDevices] = useState<IDevice[]>(tmpDeviceList)
  ...
  const { floorImgUrl, optionsDeviceStatus, optionsTurn } = useMemo(() => {
    const floorImgUrl = '/assets/images/custom/tmp-floor-map.png'
  ...
  const handleChangeHotspots = (hotspots: IDeviceHotspot[]) => {
    const newDevices = deepCopy(devices)

    for (let i = 0; i < newDevices.length; i++) {
      newDevices[i].positionX = hotspots[i].x
      newDevices[i].positionY = hotspots[i].y
    }

    setDevices(deepCopy(newDevices))
  }

  const handleToggleTurnDevice = (idx: number, isOn: boolean) => {
    const newDevices = deepCopy(devices)
    newDevices[idx].isOn = isOn
                  ...
                  >
                    ...
                    >
                      <MaterialUISwitch
                        checked={isOn}
                        disabled={disabled}
                        onChange={() => handleToggleTurnDevice(idx, !isOn)}
                      />
                    </Box>
                  </Box>
                </Box>
